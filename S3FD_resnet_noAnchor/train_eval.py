...
from model import S3FD_resnet
from loader import wider_face_loader
from loss import custom_detection_loss
from metric.Metric import IOU

#set the model
embeddingnet = models.resnet18(pretrained=True)
model = S3FD_resnet.SFD_resnet(embeddingnet)

#set the dataLoader
dataLoader = wider_face_loader.WiderFaceDataLoader(os.getcwd() + os.sep +
'data/WIDER_train/images', os.getcwd() + os.sep + 'data/WIDER_train/val','data/wider_face_split/wider_face_train_bbx_gt.txt','data/wider_face_split/wider_face_val_bbx_gt.txt',batch_size=int(config['batch_size']))

#set the loss function(if you implement your own, import that custom loss class)
criterion = custom_detection_loss.getCustomDetectionLoss
