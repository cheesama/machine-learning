...
def get_loader(trail_file, test_file, batch_size=100, max_size=20000, is_train=True, data_dir=None):
    text_field = data.Field(tokenize=korean_utilizer, sequential=True)
    #label_field = data.Field(sequential=False, use_vocab=False, postprocessing=data.Pipeline(postprocess))
    label_field = data.Field(sequential=False, use_vocab=False)

    train_file_path = Path(data_dir).joinpath(train_file)
    test_file_path = Path(data_dir).joinpath(test_file)
