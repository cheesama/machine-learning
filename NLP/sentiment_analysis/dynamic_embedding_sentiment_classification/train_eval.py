...
class Sentiment_train_eval(object):
    def __init__(self, train_loader, val_loader, model, loss_fn, config, bayes_opt=False):
    ...
    def bayes_opt_train_eval(self):
        ...
        bayes_optimizer = BayesianOptimization(f=self.bayes_opt_config_wrapper, pbounds=pbounds, random_state=13) 
...
if __name__ == '__main__':
    ...
    #model ready
    embeddingnet = WordCNN(vocab_size, config)
    embeddingnet.to(device)
    model = nn.DataParallel(embeddingnet)

    loss_fn = nn.CrossEntropyLoss()

    trainer = Sentiment_train_eval(train_loader, val_loader, model, loss_fn, vars(config))
    #trainer.train_eval()
