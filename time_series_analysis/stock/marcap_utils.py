...
def marcap_date(date, targetFolder = 'data'):
    '''
    지정한 날짜의 시가총액 순위 데이터
    ...
    csv_file = targetFolder + '/marcap-%s.csv.gz' % (date.year)
...
def marcap_date_range(start, end, code=None, targetFolder = 'data'):
    '''
    지정한 기간 데이터 가져오기
    ...
    for year in range(start.year, end.year + 1):
        try:
            csv_file = targetFolder + '/marcap-%s.csv.gz' % (year)
            df = pd.read_csv(csv_file, dtype={'Code':str}, parse_dates=['Date'])
            df_list.append(df)
        except Exception as e:
            pass
